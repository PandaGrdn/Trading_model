...
def create_dip_buying_targets(data, forward_period=5, dip_threshold=-0.005, recovery_threshold=0.007):
    """
    Create targets specifically for dip-buying strategy.
    - Thresholds have been ADJUSTED to be more sensitive for intraday data.
    """
    print(f"Creating dip-buying targets with {forward_period} intervals forward period...")
    ...
    for ticker, group in grouped:
        ...
        df['Future_Max_Return'] = df['Close'].rolling(window=forward_period).max().shift(-forward_period) / df['Close'] - 1
        
        # --- THRESHOLDS ADJUSTED FOR 15-MINUTE DATA ---
        df['Is_Recent_Dip'] = (
            (df['Drop_1d'] <= dip_threshold) |
            (df['Drop_2d'] <= dip_threshold * 1.2) |
            (df['Distance_From_5d_High'] <= dip_threshold * 1.1)
        )
        df['Is_Oversold'] = (
            (df['RSI'] <= 35) |
            (df['Close'] <= df['BB_Lower']) |
            (df['Distance_From_10d_High'] <= -0.05)
        )
    ...
    print(f"Profitable dip opportunities (Target=1): {positive_signals}")
    if total_dip_opportunities > 0:
        print(f"Targeted Dip Success Rate (for training): {positive_signals/total_dip_opportunities*100:.1f}%")
...
def create_cnn_sequences(data, features, sequence_length=20):
    """
    Create sequences for CNN training from time series data
    """
    grouped = data.groupby('Ticker')
    X_sequences = []
    y_sequences = []
...
# Add this new CNN training function
def train_cnn_model(data, features, sequence_length=20, test_size=0.2):
    """
    Train CNN model for time series pattern recognition
    """
    if not HAS_TENSORFLOW:
        print("TensorFlow not available. Skipping CNN training.")
        return None, None
    
    print("Preparing CNN sequences...")
    X_sequences, y_sequences = create_cnn_sequences(data, features, sequence_length)
    
    if len(X_sequences) == 0:
        print("No sequences created for CNN training.")
        return None, None
    
    print(f"CNN sequences shape: {X_sequences.shape}")
    print(f"CNN targets shape: {y_sequences.shape}")
    
    # Time series split for CNN
    ...
    for i in range(X_train.shape[0]):
        X_train_scaled[i] = scaler_cnn.fit_transform(X_train[i])
    
    for i in range(X_test.shape[0]):
        X_test_scaled[i] = scaler_cnn.transform(X_test[i])
    ...
    class_weights = {0: 1.0, 1: len(y_train[y_train == 0]) / len(y_train[y_train == 1])} if len(y_train[y_train == 1]) > 0 else {0: 1.0, 1: 1.0}
    ...
    if len(np.unique(y_test)) > 1:
        ...
        print(f"CNN ROC AUC: {auc_score:.4f}")
    
    return model, scaler_cnn

# Modified train_dip_buying_model function (add return of CNN models)
def train_dip_buying_model(data, features, test_size=0.2, use_smote=True, smote_method='standard'):
    ...
    # Train CNN model
    cnn_model, cnn_scaler = train_cnn_model(training_data, features, sequence_length=20)
    ...
    return final_model, scaler, features, best_threshold, cnn_model, cnn_scaler

# Add CNN prediction function
def get_cnn_predictions(cnn_model, cnn_scaler, data, features, sequence_length=20):
    """
    Generate CNN predictions for the given data
    """
    if not HAS_TENSORFLOW or cnn_model is None:
    ...
    for ticker, group in grouped:
        ticker_data = group[features].fillna(group[features].mean())
        ticker_predictions = []
        
        for i in range(len(ticker_data)):
            if i < sequence_length:
                # For early data points, use average prediction
                ticker_predictions.append(0.5)
            else:
                # Create sequence
                sequence = ticker_data.iloc[i-sequence_length:i].values
                sequence_scaled = cnn_scaler.transform(sequence)
                sequence_scaled = sequence_scaled.reshape(1, sequence_length, len(features))
                
                # Get prediction
                pred = cnn_model.predict(sequence_scaled, verbose=0)[0][0]
...
# Modified generate_trading_signals function
def generate_trading_signals(model, scaler, data, features, min_confidence=0.60, cnn_model=None, cnn_scaler=None, ensemble_weight=0.3):
    """
    Generate dip-buying signals with strict risk management
    Now includes CNN ensemble predictions
    """
    signals = data.copy()
    ...
    xgb_confidence = model.predict_proba(X_scaled)[:, 1]
    
    # Get CNN predictions
    cnn_confidence = get_cnn_predictions(cnn_model, cnn_scaler, signals, features)
    
    # Ensemble predictions (weighted average)
    if cnn_model is not None and HAS_TENSORFLOW:
        signals['XGB_Confidence'] = xgb_confidence
        signals['CNN_Confidence'] = cnn_confidence
        signals['Confidence'] = (1 - ensemble_weight) * xgb_confidence + ensemble_weight * cnn_confidence
        print(f"Using ensemble predictions (XGB: {1-ensemble_weight:.1f}, CNN: {ensemble_weight:.1f})")
    else:
        signals['Confidence'] = xgb_confidence
        signals['XGB_Confidence'] = xgb_confidence
        signals['CNN_Confidence'] = np.zeros(len(signals))
...
def main(tickers, train_start_date, train_end_date, test_start_date, test_end_date=None, 
         forward_period=5, min_confidence=0.60, force_download=False, interval='15m'):
    """
    Main function to run the enhanced dip-buying pipeline with CNN integration
    """
    if test_end_date is None:
        test_end_date = datetime.now().strftime('%Y-%m-%d')
    
    print("=== ENHANCED DIP-BUYING TRADING MODEL WITH CNN ===")
    print(f"Using {interval} interval data")
    ...
    print(f"\n--- Training Dip-Buying Models (XGBoost + CNN) with {len(selected_features)} features ---")
    model, scaler, features, best_threshold, cnn_model, cnn_scaler = train_dip_buying_model(train_data, selected_features)
    
    print("\n--- Generating Ensemble Dip-Buying Signals ---")
    signals = generate_trading_signals(model, scaler, test_data, features, min_confidence=best_threshold, 
                                     cnn_model=cnn_model, cnn_scaler=cnn_scaler, ensemble_weight=0.3)
...
if __name__ == "__main__":
    ...
    # --- Configuration ---
    FORCE_API_DOWNLOAD = False  # IMPORTANT: Keep False to use cached data and avoid rate limiting
    
    # --- Define Date Ranges (matching existing cached 15m data) ---
    today = datetime.now()
    
    # Use dates that match your existing cached 15-minute data
    # Based on: AMC_BTC-USD_ETH-USD_NVDA_SOL-USD_TSLA_20250612_to_20250622_15m.pkl
    train_start_date_intraday = '2025-05-22'
    train_end_date_intraday = '2025-06-11'
    
    # Filename: ..._20250612_to_20250622_15m.pkl
    test_start_date_intraday = '2025-06-12'
    test_end_date_intraday = '2025-06-22'  # 20 days before train end
    ...
    try:
        ...
        if signals is not None and not signals.empty:
            ...
            signals_filename = os.path.join(SIGNALS_CACHE_DIR, f"signals_{test_start_date_intraday}_to_{test_end_date_intraday}.pkl")
